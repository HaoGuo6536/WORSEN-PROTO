// ============================================================================
// FloorLoopSceneSetup.cs
// ============================================================================
// PURPOSE:
//   Rebuilds the movement FloorLoop and its runtime wiring from versioned source.
...
// USAGE NOTES:
//   - Editor-only. Refuses Play Mode and refuses to rebuild a dirty FloorLoop.
//   - Creates its scene additively and restores the prior active scene; other
//     scenes are neither saved nor closed. Rebuild replaces only FloorLoop.
...
{
    [InitializeOnLoad]
    public static class FloorLoopSceneSetup
    {
        public const string ScenePath = "Assets/Scenes/FloorLoop.unity";
        private const string UiRoot = "Assets/Resources/UI/Presentation/DebugOverlay/";

        static FloorLoopSceneSetup()
        {
        ...
        private static void ValidateCaptureBeforePlay(PlayModeStateChange state)
        {
            ...
            try
            {
                if (!arenaLoaded) throw new InvalidOperationException("Open FloorLoop to validate the play-start scene.");
                var roots = arena.GetRootGameObjects().SelectMany(go => go.GetComponentsInChildren<FloorLoopSceneRoot>(true)).ToArray();
                if (roots.Length != 1 || !roots[0].isActiveAndEnabled)
                    throw new InvalidOperationException("FloorLoop requires exactly one active, enabled scene root.");
            ...
            catch (Exception exception)
            {
                ...
                Debug.LogError("FloorLoop capture provenance is not ready. " + exception.Message +
                    " Run Worsen/Scenes/2 — Build FloorLoop after saving intended edits. This check never saves assets automatically.");
            }
        }

        [MenuItem("Worsen/Scenes/2 — Build FloorLoop")]
        public static void BuildFloorLoop()
        {
            if (EditorApplication.isPlayingOrWillChangePlaymode)
                throw new InvalidOperationException("Stop Play Mode before rebuilding FloorLoop.");
            ...
            if (oldArena.IsValid() && oldArena.isLoaded && oldArena.isDirty)
                throw new InvalidOperationException("Save your FloorLoop edits before explicitly rebuilding it.");
            ...
            try
            {
                ...
                if (oldArenaWasLoaded)
                {
                    if (!EditorSceneManager.CloseScene(oldArena, true))
                        throw new InvalidOperationException("Unity could not close the saved FloorLoop for replacement.");
                    oldArenaClosed = true;
                }
                if (!EditorSceneManager.SaveScene(scene, ScenePath))
                    throw new InvalidOperationException("Unity could not save FloorLoop.");
                sceneSaved = true;
            ...
            Debug.Log("FloorLoop movement arena rebuilt with Level, Player, first-person view and recording at 60 Hz. Live acceptance remains a separate check.");
        }

        private static FloorLoopSceneRoot BuildServices(InputDriverConfig inputConfig, DebugOverlayDriverConfig overlayConfig,
            PanelSettings panel, VisualTreeAsset tree)
        {
            ...
            var root = new GameObject("FloorLoop Scene Root").AddComponent<FloorLoopSceneRoot>();
            Wire(root, "_run", run);
        ...
        private static void BuildStage(FloorLoopSceneRoot root)
        {
            var content = new GameObject("FloorLoop Level");
            var level = FloorLoopLevelSetup.Build(content.transform);
            FloorLoopLevelSetup.BuildNavigation(level);
            ...
            var hunterProfile = EnsureFloorHunterProfile();
            ...
            var floor = FloorConfigGenerator.Build(root.transform);
            var director = new GameObject("Director Service").AddComponent<DirectorManager>();
            Wire(root, "_floor", floor);
            Wire(root, "_floorConfig", FloorConfigGenerator.EnsureAssets());
            Wire(root, "_director", director);
            Wire(root, "_directorConfig", DirectorConfigGenerator.LoadOrCreateConfig());
            var serialized = new SerializedObject(root);
            serialized.FindProperty("_spawnPosition").vector3Value = FloorLoopLevelSetup.SpawnPosition;
            serialized.FindProperty("_hunterSpawnPosition").vector3Value = FloorLoopLevelSetup.HunterSpawnPosition;
            serialized.ApplyModifiedPropertiesWithoutUndo();

            var camera = new GameObject("Main Camera").AddComponent<UnityEngine.Camera>();
            camera.tag = "MainCamera";
            camera.transform.SetPositionAndRotation(FloorLoopLevelSetup.SpawnPosition + Vector3.up * 1.65f, Quaternion.Euler(0f, 90f, 0f));
        ...
        private static HunterProfile EnsureFloorHunterProfile()
        {
            var original = HunterPrefabGenerator.EnsureAssets();
            const string configPath = "Assets/Resources/ScriptableObjects/Domain/Hunter/FloorLoopHunterMotorDriverConfig.asset";
            const string profilePath = "Assets/Resources/ScriptableObjects/Domain/Hunter/FloorLoopHunterProfile.asset";
            const string prefabPath = "Assets/Prefabs/Hunter/FloorLoopHunter.prefab";
            bool newConfig = AssetDatabase.LoadAssetAtPath<HunterMotorDriverConfig>(configPath) == null;
            var config = EnsureAsset<HunterMotorDriverConfig>(configPath);
            if (newConfig)
            {
                EditorUtility.CopySerialized(RequireAsset<HunterMotorDriverConfig>(HunterPrefabGenerator.ConfigPath), config);
                var fields = new SerializedObject(config);
                fields.FindProperty("_pathSampleRadius").floatValue = 4f;
                fields.ApplyModifiedPropertiesWithoutUndo();
            }
            bool newProfile = AssetDatabase.LoadAssetAtPath<HunterProfile>(profilePath) == null;
            var profile = EnsureAsset<HunterProfile>(profilePath);
            if (newProfile) EditorUtility.CopySerialized(original, profile);
            bool existing = AssetDatabase.LoadAssetAtPath<GameObject>(prefabPath) != null;
            var prefab = PrefabUtility.LoadPrefabContents(existing ? prefabPath : HunterPrefabGenerator.PrefabPath);
            try
            {
